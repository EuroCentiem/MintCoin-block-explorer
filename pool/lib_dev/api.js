...
var redisCommands = [
    ...
    ['hgetall', config.coin + ':peer:active'],
];
...
function collectStats(){
    ...
    async.parallel({
        pool: function(callback){
			...
			log('info', logSystem, 'Pool API stats called');
				//            console.log(JSON.stringify(redisCommands))
//				console.log(JSON.stringify(replies))
        ...
        network: function(callback){
			log('info', logSystem, 'Network API stats called');
        ...
        peers: function(callback){
			log('info', logSystem, 'Peer API stats called');
        	    callback(null, {
        ...
        dev: function(callback){
			log('info', logSystem, 'Dev API stats called');
        	    callback(null, {
                ports: getPublicPorts(config.poolServer.ports),
                weight: config.poolServer.slushMining.weight
            });
		...
		log('info', logSystem, 'Config API stats called');
//        	console.log('Coin name %s ' ,config.coin)
//        	console.log('Coin symbol %s ' ,config.symbol)
//        	console.log('Pool Port %s ' ,config.poolServer.ports)
            callback(null, {
                ports: getPublicPorts(config.poolServer.ports),
...
function broadcastLiveStats(){
	...
	redisClient.hset(config.coin + ':stats','visitors', Object.keys(liveConnections).length)
...
function handleGetPeers(urlParts, response){
    redisClient.hgetall(
            config.coin + ':peer:active',
...
console.log('Dev Module loaded %s',urlParts);
console.log('API Call %s ', urlParts.pathname);
    switch(urlParts.pathname){

        case '/stats':
    		console.log('stats collection requested')
