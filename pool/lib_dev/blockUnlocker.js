...
//function to write txt files for development
...
function runInterval(){
    async.waterfall([
        ...
        function(callback){
        if (debug) console.log('A Minty blockUnlock: %s ','Get all blocks in redis')
            redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function(error, results){
                if (error){
                	...
                	if (debug) console.log('A Minty blockUnlock: %s ','Error get all block canidates')
                    callback(true);
                    return;
                }
                if (results.length === 0){
        ...
        if (debug) console.log('A Minty blockUnlock: %s ','Check if blocks are orphaned')        
        ...
        function(blocks, callback){
        if (debug) console.log('A Minty blockUnlock: %s ','Get worker shares for each unlocked block')
        ...
        function(blocks, callback){
		if (debug) console.log('A Minty blockUnlock: %s ','Handle orphaned blocks')
            var orphanCommands = [];
        ...
        function(blocks, callback){
        if (debug) console.log('A Minty blockUnlock: %s ','Handle unlocked blocks')
